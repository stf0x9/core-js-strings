...
function formatTime(minutes, seconds) {
  return minutes
    .toString()
    .padStart(2, '0')
    .concat(seconds.toString().padStart(3, ':0'));
}
...
function isPalindrome(str) {
  const strOrig = str
    ...
    .replace(/[^a-zA-Z0-9]/g, '')
    .split('');
  const strReverse = strOrig.slice().reverse();
  return strOrig.join('') === strReverse.join('');
}
...
function extractNameFromTemplate(value) {
  const [firstName, lastName] = value.substring(7).replace('!', '').split(' ');
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const letterToEncode = str[i];
    if (isLowerCaseLetter(letterToEncode)) {
      encoded.push(encodeLetter(letterToEncode));
    } else if (isUpperCaseLetter(letterToEncode)) {
      encoded.push(encodeLetter(letterToEncode.toLowerCase()).toUpperCase());
    } else {
      encoded.push(letterToEncode);
    }
  }
  return encoded.join('');
}
